...
export interface DeployConfigInput {
  ...
  withAssets?: boolean; // optional flag
  cleanupAuth?: boolean; // optional flag
}
...
export function validateDeployConfig(
  ...
  const environment = input.environment?.toLowerCase() as
    | 'stage'
    | 'live'
    | undefined;
  const action = input.action?.toLowerCase() as
    | 'install'
    | 'update'
    | undefined;
  ...
  if (!environment || !validEnvironments.includes(environment)) {
    errors.push(
      `Invalid or missing environment. Expected one of: ${validEnvironments.join(
        ', '
      )}.`
    );
  }
  ...
  if (!action || !validActions.includes(action)) {
    errors.push(
      `Invalid or missing action. Expected one of: ${validActions.join(', ')}.`
    );
  }
