...
export function resolveProjectPaths(input) {
    ...
    if (environment === 'stage') {
        inventoryFile = path.join(projectRoot, 'ansible/core/inventories/stage/inventory.yml');
        playbookFile = path.join(projectRoot, 'ansible/core/stage-deploy.yml');
        vaultFile = path.join(projectRoot, 'ansible/core/inventories/stage/group_vars/server.yml');
    }
    else if (environment === 'live') {
        inventoryFile = path.join(projectRoot, 'ansible/core/inventories/production/inventory.yml');
        playbookFile = path.join(projectRoot, 'ansible/core/live-deploy.yml');
        vaultFile = path.join(projectRoot, 'ansible/core/inventories/production/group_vars/server.yml');
    }
    else {
